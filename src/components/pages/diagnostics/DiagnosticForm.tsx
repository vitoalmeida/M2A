...
const DiagnosticForm: React.FC<Props> = ({ closeForm, diagnostic }) => {
  ...
  function returnRisk() {
    if (total < 24) {
      return <span className="text-red-800">Extremo</span>;
    } else if (total < 48) {
      return <span className="text-red-500">Alto</span>;
    } else if (total < 72) {
      return <span className="text-orange-400">Médio</span>;
    } else if (total < 96) {
      return <span className="text-yellow-400">Baixo</span>;
    } else {
      return <span className="text-green-400">Nenhum</span>;
    }
  }
